...
//
import {CollectionView, format, DateTime, isFunction} from '@grapecity/wijmo';
...
export interface AppData {
  ...
  groupedOrders: CollectionView;
  pagedOrders: CollectionView;
  addNewOrders: CollectionView;
  ldOneLine: Cell[];
  ldTwoLines: Line[];
  ldThreeLines: Line[];
  layoutDefs: CollectionView;
}

@Injectable({
  providedIn: 'root'
})
export class MultirowResizeService {
  ...
  getData(): AppData {
    ...
    let today = new Date();
    for (let i = 0; i < 20; i++) {
      let shipped = DateTime.addDays(today, -randBetween(1, 3000));
      orders.push({
        id: i,
    ...
    function clone(obj: any) {
      if (isFunction(Object.assign)) { // IE does not support it
    ...
    appData.orders = new CollectionView(orders);

    // expose grouped orders CollectionView to the app
    appData.groupedOrders = new CollectionView(orders, {
      groupDescriptions: [
        'customer.city'
      ]
    });
    ...
    appData.pagedOrders = new CollectionView(orders, {
      pageSize: 4
    });

    // expose addNew oders CollectionView to the app
    appData.addNewOrders = new CollectionView(orders, {
